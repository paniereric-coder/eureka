param(
    ...
    [string]$CredentialPath = (Join-Path ([Environment]::GetFolderPath('ApplicationData')) "Eureka" "varennes-credential.xml"),
...
function Ensure-Pillow {
    ...
    if (-not $python) {
        throw @"
python3 est introuvable.

Installe Python 3 (par exemple avec Homebrew : brew install python), puis Pillow avec :
    python3 -m pip install pillow
"@
    }

    & python3 -c "import PIL" 2>$null
...
function Ensure-Pikepdf {
    & python3 -c "import pikepdf" 2>$null
    if ($LASTEXITCODE -ne 0) {
...
function Download-EurekaDocument {
    ...
    $pythonScript = Join-Path ([System.IO.Path]::GetTempPath()) ("eureka_assemble_{0}.py" -f [Guid]::NewGuid().ToString("N"))
